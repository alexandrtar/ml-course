...
# Global variables for loaded models - исправляем инициализацию
model_state = {
    'model': None,
    'feature_names': None,
    'loaded': False
...
def load_models():
    ...
    try:
        ...
        if os.path.exists(model_path):
            ...
            logger.info(f"   Model type: {type(model_state['model']).__name__}")
            if hasattr(model_state['model'], 'n_features_in_'):
                logger.info(f"   Model features: {model_state['model'].n_features_in_}")
        else:
        ...
        if os.path.exists(feature_names_path):
            with open(feature_names_path, 'r', encoding='utf-8') as f:
                model_state['feature_names'] = [line.strip() for line in f.readlines()]
            logger.info(f"✅ Loaded {len(model_state['feature_names'])} feature names")
            logger.info(f"   First 5 features: {model_state['feature_names'][:5]}")
        else:
            logger.error(f"❌ Feature names file not found: {feature_names_path}")
            return False
            
        # Verify model and features match
        if (hasattr(model_state['model'], 'n_features_in_') and 
            model_state['model'].n_features_in_ != len(model_state['feature_names'])):
            logger.warning(f"⚠️ Feature count mismatch: model expects {model_state['model'].n_features_in_}, but we have {len(model_state['feature_names'])} features")
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("api.app_final:app", host="0.0.0.0", port=8000, reload=True)