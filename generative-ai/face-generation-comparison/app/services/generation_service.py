...
class GenerationService:
    ...
    def interpolate_images(self, model_type: str, start_vector: List[float] = None, 
        ...
        if start_vector is None:
            ...
            # Проверяем длину вектора
            if len(start_vector) != model.latent_dim:
                raise ValueError(f"Start vector must have length {model.latent_dim}, got {len(start_vector)}")
    
        if end_vector is None:
            end_vector = model.generate_latent_vector(1)[0].tolist()
        else:
            # Проверяем длину вектора
            if len(end_vector) != model.latent_dim:
                raise ValueError(f"End vector must have length {model.latent_dim}, got {len(end_vector)}")
    
        # Преобразуем в numpy массивы
        start_np = np.array(start_vector)
        end_np = np.array(end_vector)
    
        # Убедимся, что векторы имеют правильную форму (1, latent_dim)
        if start_np.ndim == 1:
            start_np = start_np.reshape(1, -1)
        if end_np.ndim == 1:
            end_np = end_np.reshape(1, -1)
    
        # Проверка размерности для отладки
        print(f"Debug: start_np shape = {start_np.shape}, expected (1, {model.latent_dim})")
        print(f"Debug: end_np shape = {end_np.shape}, expected (1, {model.latent_dim})")
